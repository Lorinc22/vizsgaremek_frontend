  ...
  loadRestaurants = async () => {
    ...
    for (let i = 0; i < loadedRestaurants1.length; i++) {
      elements1.push(
        ...
        >
          <img
            className="pizzaKingImg"
            src={loadedRestaurants1[i].url}
            key={i}
            onClick={() =>
              this.handleCardClick(this.state.loadedRestaurants1[i].id)
            }
          />
    ...
    for (let i = 0; i < loadedRestaurants2.length; i++) {
      elements2.push(
        <div
          className="HomePageCard"
          key={loadedRestaurants2[i].id}
          style={{ backgroundColor: "#3c1945" }}
        >
          <img
            className="pizzaKingImg"
            src={loadedRestaurants2[i].url}
            key={i + 4}
            onClick={() =>
              this.handleCardClick(this.state.loadedRestaurants2[i].id)
            }
          />
